"""
A.R.C SENTINEL - Main FastAPI Server
=====================================
SOC Platform Backend with Real-time Event Processing
...
async def lifespan(app: FastAPI):
    ...
    print(f"[STARTUP] Telemetry generation: Every {settings.TELEMETRY_INTERVAL_SECONDS}s")
...
async def resolve_incident(
    ...
    # Broadcast resolution over WebSocket
    await ws_manager.broadcast({
...
async def simulate_attack(
    ...
    async def inject_attack_chain():
        nonlocal incident_created
        for event in chain:
            incident = await process_event(event)
            if incident:
                incident_created = True
            await asyncio.sleep(0.5)  # Slight delay between events
